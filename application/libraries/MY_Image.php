...
class MY_Image{
    ...
    public function resize($x=-1,$y=-1,$w=-1,$h=-1)
    {
        ...
        //resize的时候自定义参数
        if($x != -1 && $y!=-1 && $h!=-1 && $w!=-1){
            return $this->resizeImage($x,$y,$h,$w);
        }
    ...
    private function resizeImage($x,$y,$h,$w)
    {
        $func_output = 'image' . $this->image_type[$this->image_type_index[$this->image_ext]];
...
//             clipping method 1: left or top 2: middle 3: right or bottom
        ...
        $this->source_image_x = $x;
        $this->source_image_y = $y;

        $this->source_image_w = $w;
        $this->source_image_h = $h;

//        var_dump($this->width,$this->height);die;
//		var_dump($this->source_image_w,$this->source_image_h,$this->source_image_x,$this->source_image_y);die;

        $dst_img = imagecreatetruecolor($this->width, $this->height);

//        var_dump($dst_img, $im, $dst_x, $dst_y, $this->source_image_x, $this->source_image_y, $fdst_w, $fdst_h, $this->source_image_w, $this->source_image_h);die;
